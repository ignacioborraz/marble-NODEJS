...
const controller = {
    ...
    all: async(req,res,next) => {
        ...
        try {
            let all = await Note.find(query).populate('client','name company -_id').sort(order)
            if (!all.length) {
            ...
            let data = number_codes.map(number_code => {
                ...
                let client = all.find(one => one.number_code === number_code).client.company || all.find(one => one.number_code === number_code).client.name
